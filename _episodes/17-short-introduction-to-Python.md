...
Since we're going to be doing a little more Python than I initially planned, we're going to move the conda install.

```UNIX
mv Miniconda2 /work/your_user_directory
```

And update your .bashrc file:

```
export PATH="/work/yourUserName/Miniconda2/bin:$PATH"
```

The extra packages we'll install will make the conda install a little too big for the home directory.
...
The above commands will install Pandas, a library for manipulating datasets, and ggplot, a visualization library. 

We'll also real quick update the poretools install so it knows where our new Python is. Move your poretools into your work directory, enter it and type:
